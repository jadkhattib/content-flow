...
export default function ChatBubble({ data }: ChatBubbleProps) {
  ...
  useEffect(() => {
    ...
    if (savedMessages) {
      try {
        ...
        // Ensure parsedMessages is an array before setting
        if (Array.isArray(parsedMessages)) {
          setMessages(parsedMessages);
          setIsInitialized(true);
        } else {
          console.warn('Invalid chat messages format in localStorage, resetting');
          setMessages([]);
        }
      } catch (error) {
  ...
  async function send() {
    ...
    const nextMessages = [...currentMessages, { role: "user" as const, content: text }];
    ...
    try {
      ...
      setMessages((m) => [...(Array.isArray(m) ? m : []), { role: "assistant", content: responseData.reply ?? "" }]);
      setIsInitialized(true);
    } catch (error) {
      setMessages((m) => [...(Array.isArray(m) ? m : []), { role: "assistant", content: "Sorry, I encountered an error. Please try again." }]);
    } finally {
      setIsTyping(false);
    }
  }
  ...
  return (
    <>
      ...
      >
        ...
        {Array.isArray(messages) && messages.length > 0 && showMessages && (
        ...
        <div className="max-w-4xl mx-auto px-6 pb-16">
          <div className="relative">
            ...
            {(!showMessages || !Array.isArray(messages) || messages.length === 0) && (
              <>
                <div className="absolute inset-0 rounded-full border-2 border-purple-500/40 blur-sm" style={{
                  animation: 'purplePulse 3s ease-in-out infinite'
                }}></div>
            ...
            <div className={clsx(
              ...
              <input
                ...
                placeholder={`Ask Mochi about ${data.brandName} analysis, insights...`}
